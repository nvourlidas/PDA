...
<div class="topnav">
  <a  class="active"  href="menu1.php" style="width:47%">Menu</a>
  <a  href="add-product.html" style="width:47%">Add Product</a>
</div>
<br>
...
<?php
  ...
  while($row = $result->fetch_assoc()){
    ...
    elseif ($row['product_category']==3) {
      ...
      $category1="Chocolate";
    }elseif ($row['product_category']==5) {
      $category1="Tea";
    }elseif ($row['product_category']==6) {
      $category1="Pastry";
    }

    echo"<tr><form action=mod-product.php method=post>";
    echo"<td><input type=text name=name value='".$row['product_name']."'></td>";
    echo"<td>".$category1."</td>";
    echo"<td><input type=text name=price value='".$row['product_price']."'></td>";
    echo"<input type=hidden name=id value='".$row['Product_Code']."'>";
    echo"<td><button type=submit>Modify</button>";
    echo"</form></tr>";
}
